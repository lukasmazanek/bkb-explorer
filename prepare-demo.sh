#!/bin/bash
#
# BKB Explorer - RBCZ Data Preparation Script
#
# Copies RBCZ ontology data from ontology-lift/export and bundles it
# into js/data-rbcz.js for local use.
#
...
OUTPUT_FILE="$SCRIPT_DIR/js/data-rbcz.js"
ONTOLOGY_LIFT_DIR="$SCRIPT_DIR/../ontology-lift"

echo "🔐 BKB Explorer - Preparing RBCZ data (private, never committed)..."
echo ""

# Check if ontology-lift exists
if [ ! -d "$ONTOLOGY_LIFT_DIR" ]; then
    echo "❌ Error: ontology-lift directory not found at $ONTOLOGY_LIFT_DIR"
    exit 1
fi
...
if [ ! -d "$EXPORT_DIR" ]; then
    echo "❌ Error: No export directory found. Run ontology-lift pipeline first."
    exit 1
fi
...
# Generate data-rbcz.js
echo "📝 Generating $OUTPUT_FILE..."
echo ""

cat > "$OUTPUT_FILE" << 'HEADER'
/**
 * BKB Explorer - RBCZ Data (PRIVATE)
 *
...
# RBCZ Domains hierarchy
echo "// RBCZ Domain hierarchy (private)" >> "$OUTPUT_FILE"
echo "const RBCZ_DOMAINS_DATA = {" >> "$OUTPUT_FILE"
...
if [ -f "$RETAIL_FILE" ]; then
    ...
    echo "  ✅ Retail: $RET_COUNT concepts"
else
    echo '{ "domain": { "name": "Retail", "path": "RBCZ:Retail" }, "concepts": [] }' >> "$OUTPUT_FILE"
    echo "  ⚠️  Retail: not found"
fi
echo ";" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"

# Export as BKB_RBCZ_DATA (merged with BKB_DATA by app.js)
echo "// Export for application (merged with BKB_DATA by app.js)" >> "$OUTPUT_FILE"
echo "window.BKB_RBCZ_DATA = {" >> "$OUTPUT_FILE"
echo "  domains: RBCZ_DOMAINS_DATA," >> "$OUTPUT_FILE"
echo "  investment: INVESTMENT_DATA," >> "$OUTPUT_FILE"
echo "  payments: PAYMENTS_DATA," >> "$OUTPUT_FILE"
echo "  retail: RETAIL_DATA" >> "$OUTPUT_FILE"
echo "};" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
echo "console.log('🔐 RBCZ data loaded:', Object.keys(window.BKB_RBCZ_DATA));" >> "$OUTPUT_FILE"
...
echo "   ⚠️  This file is gitignored - never commit!"
echo ""
echo "🚀 Ready! Open index.html in browser."
